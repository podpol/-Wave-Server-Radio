 ...
 * License: AGPL-3.0
 * 
 * The server NEVER hears your voice and NEVER stores file contents.
 * Audio and Files flow P2P via WebRTC. Server only handles signaling and metadata.
 * 
 * NEW in v2.0:
 ...
 *  - Redirect to home server for remote channels
 * 
 * ENV:
 *   PORT=3000
 *   FEDERATION_ID=server-01
 *   FEDERATION_URL=ws://YourIP:3001
 *   FEDERATION_PEERS=ws://berlin.example.com:3001,ws://tokyo.example.com:3001
 *   FEDERATION_SECRET=my-shared-secret
 */

'use strict';

// ✅ Грузим .env ДО чтения любых process.env — без этого файл .env.example
// был бы просто мёртвым текстом, который никто никогда не читает
try { require('dotenv').config(); } catch (e) { console.warn('⚠️  dotenv not installed — run `npm install dotenv` to use a .env file. Falling back to real environment variables only.'); }
    ...
    channels.forEach((ch, id) => {
      data.push({
        ...
        chest: (ch.chest || []).map(file => ({
          ...
          uploader: file.uploader, uploaderName: file.uploaderName, timestamp: file.timestamp
        }))
      });
    });
...
const app = express();
if (helmet) app.use(helmet({ contentSecurityPolicy: false })); // CSP тут не нужен, это API/WS сервер, не сайт с формами
app.set('trust proxy', 1); // если сервер стоит за nginx/Caddy — иначе req.ip будет адресом прокси, а не клиента
...
const io = new Server(server, {
  cors: {
    origin: (origin, callback) => {
      // ✅ Убрали безусловное разрешение origin 'null' — оно слишком широкое
      // (его шлют file://, sandboxed iframe и т.п., не только расширение).
      // Расширение всегда шлёт chrome-extension://<id>, это и проверяем.
...
// ✅ Не даём случайно уйти в прод с дефолтным ботовым токеном
...
const relayHelpers = new Map(); // socketId -> { userId, since } — просто статистика, без привилегий
...
const federation = new Federation({
  ...
  // ✅ FEDERATION_PEERS — обязательные жёсткие связи (например, между вашими VPS).
  // ✅ FEDERATION_BOOTSTRAP — для новых/волонтёрских узлов: достаточно ОДНОГО известного
  //    адреса, дальше сеть доразведает себя сама через gossip внутри federation.js.
...
function securityLog(event, details) {
  console.warn(`[SECURITY] [${new Date().toISOString()}] ${event}: ${JSON.stringify(details)}`);
}
...
// Периодическая рассылка обновлённых данных каналов пирам
setInterval(() => {
  try {
    channels.forEach((ch) => {
      federation.broadcastChannelUpdated(ch);
    });
  } catch(e) {}
}, 10000);
...
io.on('connection', (socket) => {
  ...
  // ── GET CHANNELS (глобальный список через федерацию) ──
  socket.on('get-channels', safeHandler('get-channels', (cb) => {
    // ✅ Возвращаем ГЛОБАЛЬНЫЙ список (локальные + удалённые)
    cb(federation.getGlobalChannelList());
  }));

  // ── CREATE CHANNEL (с глобальной проверкой имени) ──
  socket.on('create-channel', safeHandler('create-channel', async (rawData, cb) => {
    ...
    // ✅ ГЛОБАЛЬНАЯ ПРОВЕРКА УНИКАЛЬНОСТИ ИМЕНИ
    const nameTaken = await federation.isNameTakenGlobally(channelName);
    if (nameTaken) return callback({ error: `Channel name "${channelName}" is already taken globally` });
  ...
  // ── JOIN CHANNEL (с redirect для удалённых каналов) ──
  socket.on('join-channel', safeHandler('join-channel', (rawData, cb) => {
    ...
    // ✅ ПРОВЕРКА: канал локальный или удалённый?
    if (!federation.isLocalChannel(channelId)) {
      ...
      if (homeUrl) {
        return callback({
          ...
          redirectUrl: homeUrl,
          channelId,
          message: 'This channel is on another server. Redirecting...'
        });
      }
  ...
  socket.on('chest-add-file', safeHandler('chest-add-file', (rawData, cb) => {
    ...
    if (metadata.size > 150 * 1024 * 1024) return callback({ error: 'File exceeds 150MB' });
  ...
  socket.on('start-vote', safeHandler('start-vote', (rawData, cb) => {
    ...
    votes.set(voteId, { id: voteId, channelId: userData.channelId, targetUserId: data.targetUserId, targetName: sanitize(data.targetName, 20), yes: new Set([userData.userId]), no: new Set(), votedIps: new Set([socket._clientIp]), expiresAt: Date.now() + CONFIG.limits.VOTE_DURATION });
  ...
  socket.on('cast-vote', safeHandler('cast-vote', (rawData) => {
    ...
    if (!vote) return;
    const userData = users.get(socket.id);
    if (!userData || vote.votedIps.has(socket._clientIp)) return;
    vote.votedIps.add(socket._clientIp);
  ...
  // ── FEDERATION STATUS (для вкладки "Network" в клиенте) ──
  socket.on('get-federation-status', safeHandler('get-federation-status', (cb) => {
    if (typeof cb !== 'function') return;
    cb(federation.getStatus());
  }));

  // ── RELAY-MODE (просто статистика "узлов-помощников", без выдачи привилегий) ──
  ...
  // ── TURN — временные креденшелы (не даём постоянный логин/пароль в клиент) ──
  socket.on('get-turn-credentials', safeHandler('get-turn-credentials', (cb) => {
    ...
    if (!turnSecret || !turnUrls.length) return cb({ iceServers: [] }); // TURN не настроен — клиент останется на STUN
...
server.listen(CONFIG.port, CONFIG.host, () => {
  ...
  // ✅ Инициализируем федерацию после старта HTTP сервера
